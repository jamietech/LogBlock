...
public class InteractLogging extends LoggingListener
{
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		final WorldConfig wcfg = Config.getWorldConfig(event.getPlayer().getWorld());
		if (wcfg != null) {
			...
			switch (type) {
				...
				case SOIL:
					if (wcfg.isLogging(Logging.CROPTRAMPLE) && (event.getAction() == Action.PHYSICAL)) {
						// 3 = Dirt ID
						this.consumer.queueBlock(player.getName(), loc, typeId, 3, blockData);
						...
						if (BukkitUtils.getCropBlocks().contains(trampledCrop.getType())) {
							this.consumer.queueBlockBreak(player.getName(), trampledCrop.getState());
						}
					}
					break;
			}
